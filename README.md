...
2. Уметь сохранять и загружать данные: работа с файлами и простыми интерфейсами вроде Windows Forms.
3. Уметь работать с системой контроля версий Git.
4. Иметь представление о реляционных базах данных и работе с PostgreSQL.
5. Уметь писать юнит-тесты для .NET-приложений.
6. Знать основы HTML, CSS и JavaScript для создания веб-страниц.
7. Понимать принцип открытости/закрытости и паттерн "Стратегия".
8. Уметь создавать веб-API на базе ASP.NET Core.
9. Иметь навыки работы с файлами и их анализа.
10. Уметь работать с XML и CSV форматами данных.
11. Понимать концепции асинхронности и параллелизма в .NET.
12. Понимать, когда стоит перестать копировать файлики на боевой сервер руками.
13. Понимать, что такое ORM, и уметь работать с Entity Framework.
14. Понимать, как можно потестировать UI браузера.
...
1. **Порядок имеет значение**: уроки выстроены по нарастанию сложности, новичку лучше идти по порядку. Но самодостаточные
   темы (Git, PostgreSQL, CI/CD) можно смотреть отдельно.
...
- Базовые знания компьютера и желание учиться и практиковаться.
- **Windows**: уроки 1, 2 и 9 (WinForms / .NET Framework) запускаются только под Windows.
- Установленная среда разработки **Visual Studio 2022** (достаточно Community) либо Rider / VS Code с установленным
  .NET 8 SDK.
- Доступ к сети Интернет для просмотра видео-уроков и поиска дополнительной информации.
- Для отдельных уроков понадобится больше: урок 13 (EF) — SQL Server (хватит Express) и Node.js; урок 15 (Playwright) —
  установка браузеров (`pwsh playwright.ps1 install`).
